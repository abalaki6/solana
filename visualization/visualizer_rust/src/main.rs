...
/**
    ...
    for node in &nodes {
        println!("New Node: ");
        let printable = format!("{}", &node.id);
        println!("Pubkey: {}", &printable);
        println!("tpu: {}", &node.tpu.to_string());
        println!("tvu: {}", &node.tvu.to_string());
        println!("storage address: {}", &node.storage_addr.to_string());
        println!("RPC: {}", &node.rpc.to_string());
    }
    println!("{}", &nodes.len());
